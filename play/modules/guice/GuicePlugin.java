...
public class GuicePlugin extends PlayPlugin implements BeanSource {
	...
	private void loadCustomInjector(Class clazz) throws InstantiationException,
	...
	private void loadInjectorFromModules() {
		if (modules.isEmpty()) {
			throw new IllegalStateException(
					"Could not find any custom guice injector or abstract modules. Are you sure you have at least one on the classpath?");
		}
	...
	private void injectAnnotated() {
		try {
			for (Class<?> clazz : Play.classloader
					.getAnnotatedClasses(play.modules.guice.InjectSupport.class)) {
	...
	private void inject(Field field) throws IllegalAccessException {
		...
		if (fieldBinding != null) {
			field.set(null,
					getBeanWithKey(Key.get(field.getType(), fieldBinding)));
		} else {
	...
	private Annotation fieldBinding(Field field) {
		for (Annotation annotation : field.getAnnotations()) {
			...
			for (Annotation internal : annotation.annotationType()
					.getAnnotations()) {
